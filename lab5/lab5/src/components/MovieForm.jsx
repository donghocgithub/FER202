...
import { Form, Button, Row, Col, Table, Image, Modal } from 'react-bootstrap';
    ...
    <Row className="mb-3">
      <Col md={6}>
        <Form.Group controlId="formAvatar">
          ...
          type="invalid">{errors.avatar}
          </Form.Control.Feedback>
          {imagePreview && (<div className="mt-2">
            <Image
           src={imagePreview} alt="Preview" thumbnail
      ...
      <Col md={6}>
        <Form.Group controlId="formTitle">
          ...
          onChange={handleInputChange} 
          placeholder="Tên phim" required isInvalid={validated && errors.title} 
    ...
    <Row className="mb-3">
      <Col md={12}>
        <Form.Group controlId="formDescription">
          ...
          onChange={handleInputChange} 
          placeholder="Mô tả phim" required isInvalid={validated && errors.description} 
    ...
    <Row className="mb-3">
      ...
      <Col md={4}>
        <Form.Group controlId="formDuration">
          ...
          onChange={handleInputChange} 
          placeholder="Phút" required min="1" max="600" 
      ...
      <Col md={2}>
        <Form.Group controlId="formYear">
          ...
          value={currentMovie.year || ''}
           onChange={handleInputChange} 
           placeholder="Năm" required min="1900" max="2030" 
...
const MovieForm = () => {
  ...
  const validateForm = () => {
    ...
    (currentMovie.description.length < 10) 
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSend = { ...currentMovie, duration: parseInt(currentMovie.duration || 0), year: parseInt(currentMovie.year || 0), genreId: parseInt(currentMovie.genreId || 1) };
    const success = await handleCreateOrUpdate(dataToSend, isEditing !== null, isEditing);
    if (success) {
      if (isEditing === null) {
        setImagePreview(''); setValidated(false); setErrors({});
      } else {
  ...
  return (
    <>
      {/* Render the create form as a bordered table below the movie list */}
      <Table bordered className="mb-4 bg-white">
        <thead>
          <tr>
            <th className="p-3">📽️ Thêm Phim Mới</th>
          </tr>
        </thead>
        <tbody>
          <tr>
