...
export const AuthProvider = ({ children }) => {
    // Try to restore user from localStorage so reloads don't sign out the user
    const storedUser = (() => {
        try {
            const user = JSON.parse(localStorage.getItem('user'));
            // Nếu không phải object hoặc không có các trường cần thiết thì xoá
            if (!user || typeof user !== 'object') {
                localStorage.removeItem('user');
                return null;
            }
            // Nếu thiếu các trường quan trọng thì remove và trả về null
            if (!('username' in user) || !('role' in user) || !('status' in user)) {
                localStorage.removeItem('user');
                return null;
            }
            // Kiểm tra nếu user bị locked hoặc blocked, tự động logout
    ...
    const initial = { ...initialAuthState, isAuthenticated: !!storedUser, user: storedUser };
    ...
    const login = async ({ usernameOrEmail, password }) => {
        ...
        try {
            ...
            if (user) {
                // Nếu tài khoản không active thì trả về lỗi rõ ràng và không login
                if (user.status !== 'active') {
                    const msg = user.status === 'blocked' || user.status === 'locked'
                ...
                // Nếu user tồn tại và active thì tiếp tục login
