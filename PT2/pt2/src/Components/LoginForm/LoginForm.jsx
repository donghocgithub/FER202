...
import { useNavigate, useLocation } from 'react-router-dom'; // Thêm useNavigate để chuyển hướng
...
function LoginForm() {
  ...
  // external message (from redirects) e.g. blocked / unauthorized
  const [externalMsg, setExternalMsg] = useState(location.state?.message || null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Chỉ hiển thị modal khi login thành công
      if (result && result.success) {
        dispatch({ type: 'SHOW_SUCCESS_MODAL' });
      } else {
        // Nếu API trả về kết quả thất bại, hiển thị message rõ ràng trong form
        const msg = (result && result.error) || 'Login failed';
        dispatch({ type: 'SET_ERRORS', errors: {} });
        // cập nhật trạng thái thông báo hiển thị trên form
        dispatch({ type: 'SET_ERROR', field: 'status', message: msg });
      }
    } catch (err) {
      // Lỗi mạng hoặc lỗi không xác định
      console.error('Login error:', err);
      dispatch({ type: 'SET_ERROR', field: 'status', message: err.message || 'Login error' });
    }
  };
  //9. Xử lý reset form
    const handleReset = () => { 
      ...
      <Row className="justify-content-md-center">
        <Col xs={12} md={6}>
          <Card>
            ...
            <Card.Body>
              {/* Hiển thị một thông báo duy nhất ưu tiên: externalMsg > formState.errors.status > auth error */}
              {(() => {
                const msg = externalMsg || formState.errors?.status || error;
                if (!msg) return null;
                const handleClose = () => {
                  if (externalMsg) setExternalMsg(null);
                  else if (formState.errors?.status) dispatch({ type: 'CLEAR_ERROR', field: 'status' });
                  else if (error) clearError();
                };
                return (
                  <Alert variant="danger" className="mb-3" onClose={handleClose} dismissible>
                    {msg}
                  </Alert>
                );
              })()}
